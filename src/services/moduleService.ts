...
export class ModuleService implements IModuleService {
  ...
  async createModule(data: Partial<IModule>): Promise<void> {
    ...
    const moduleId= await this.moduleRepository.createModule(moduleData);
        await this.courseRepository.addModuleToCourse(data.courseId, moduleId);
  ...
  async deleteModule(moduleId: string, courseId: string): Promise<void> {
       // Remove moduleid reference from course
    await this.courseRepository.removeModuleFromCourse(courseId, moduleId);
