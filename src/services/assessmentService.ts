...
console.log('quizzes',quizzes)
    ...
    const assessmentRecord = quizzes.map((quiz) => {
      ...
      const mergedOptions = quiz.options.map((opt) => ({
        option: opt.text,
        isCorrect: opt.is_correct,
        isSelected: userAnswer?.options.some((uo) => uo.option === opt.text) || false,
      }));
...
console.log('assessmnetrecord',assessmentRecord)
    // 3. Create assessment
    ...
    let report = await this.reportRepository.getReportByCourseAndUser(courseId, userId);
    if (!report) {
      ...
      report.quizAssessment = assessment._id;
